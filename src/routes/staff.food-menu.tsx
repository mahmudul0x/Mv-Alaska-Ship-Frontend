...
function FoodMenuPage() {
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <PageHeader title={t("fm.title")} subtitle={t("fm.subtitle")}>
      ...
      {isLoading ? (
        <div className="p-16 flex items-center justify-center gap-3 text-muted-foreground">
          <Loader2 className="size-5 animate-spin text-gold" /> {t("fm.loading")}
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <StatCard
              ...
              tone="emerald"
            />
            <StatCard label={t("common.hidden")} value={String(stats.hidden)} icon={EyeOff} />
            <StatCard
              label={t("fm.perDay")}
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            >
              <Copy className="size-3.5" /> {t("fm.copyDay")}
            </button>
          </div>
          ...
          {items.length === 0 && (
            <div className="rounded-2xl border border-dashed border-border bg-card p-16 text-center space-y-3">
              ...
              <p className="text-sm text-muted-foreground">{t("fm.noItems")}</p>
            </div>
          )}
        </div>
      )}
    ...
    <DialogShell title={`Copy ${fromLabel}`} onClose={onClose}>
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">
          {t("fm.duplicateAll")} <strong className="text-foreground">{source.length}</strong>{" "}
          dish(es) from {fromLabel} into another day. Existing items on the target day are kept.
        </p>
        <StaffField label={t("fm.copyInto")}>
          <select
            value={toDay}
...
function AddItemDialog({
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <StaffField label={t("fm.itemName")}>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder={t("fm.itemPlaceholder")}
            className={staffInputClass}
            autoFocus
          />
        </StaffField>
...
function EditItemDialog({
  ...
  return (
    <DialogShell title={t("fm.editItem")} onClose={onClose}>
      <div className="space-y-4">
        <StaffField label={t("fm.itemName")}>
          <input
            value={name}
